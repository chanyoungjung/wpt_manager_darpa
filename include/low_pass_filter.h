...
public:
    ...
    template <typename T>
    void getFilteredValue(T &data);
    template <typename T>
    void getFilteredArray(std::vector<T> raw_array, std::vector<T> &filtered_array);
    template <typename T>
    bool initLpfValue(T data, bool only_on_first_call);
    void setFirstCallTrue();
...
template <typename T>
void low_pass_filter::getFilteredValue(T &data)
{
    initLpfValue(data, true);
    double raw_data_d   = (double) data;
    data                = (T) (weight_ * raw_data_d + (1-weight_) * last_filtered_);
    last_filtered_      = (double) data;
}
...
template <typename T>
bool low_pass_filter::initLpfValue(T data, bool only_on_first_call)
{
    if ((only_on_first_call && first_call_) || !only_on_first_call){
        last_filtered_ = (double) data;
    }
    first_call_ = false;
}

void low_pass_filter::setFirstCallTrue()
{
    first_call_ = true;
}
